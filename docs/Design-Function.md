...
- The header and footer use a fixed overflow-hidden brand viewport to crop the JPEG's built-in white margins. The image is scaled to `124px` on desktop and `100px` on mobile, with centered offsets that leave extra source-image clearance around the complete circular mark and upper `TRIPLE` lettering. It uses `mix-blend-mode: multiply` so the remaining white JPEG background blends into the paper surface instead of appearing as a white rectangle. This keeps the existing logo asset, alt text, header height, footer layout, and mobile navigation behavior unchanged.
